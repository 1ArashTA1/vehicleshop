...
local StringCharset = {}
local NumberCharset = {}
RegisterNetEvent('pa-vehicleshop:buyVehicle:server', function(type, vehicle, price, dealershipId, sender, job)
    local src = source
    ...
    TriggerClientEvent('pa-vehicleshop:buyVehicle:client', src, vehicle, plate, dealershipId)
...
CreateCallback('pa-vehicleshop:generatePlate:server', function(source, cb)
    local plate = GeneratePlate()
    cb(plate)
end)
...
-- Stocks
RegisterNetEvent('pa-vehicleshop:updateDealershipStockData:server', function(dealershipId, stocksData)
    local dealershipData = MySQL.query.await('SELECT * FROM pa_vehicleshop_stocks WHERE dealershipId = ?', {dealershipId})
...
CreateCallback('pa-vehicleshop:getVehStock:server', function(source, cb, dealershipId)
    local dealershipData = MySQL.query.await('SELECT * FROM pa_vehicleshop_stocks WHERE dealershipId = ?', {dealershipId})
    local anusVal = {}
    for k, v in pairs(dealershipData) do
        for a, b in pairs(v) do
...
local testVehicles = {}
RegisterNetEvent('pa-vehicleshop:startTest:server')
AddEventHandler('pa-vehicleshop:startTest:server', function(netId)
    testVehicles[netId] = {
        playerId = source
    }
end)

RegisterNetEvent('pa-vehicleshop:finishTest:server')
AddEventHandler('pa-vehicleshop:finishTest:server', function(netId)
...
AddEventHandler('entityRemoved', function(entity)
    ...
    if testVehicles[netId] then
        TriggerClientEvent('pa-vehicleshop:finishTest:client', testVehicles[netId].playerId)
        testVehicles[netId] = nil
    end
end)

RegisterNetEvent('pa-vehicleshop:updateShowroomVehicles:server', function(dealershipId, data)
    local src = source
...
CreateCallback('pa-vehicleshop:getShowroomData:server', function(source, cb, dealershipId)
    local showroomTable = {}
...
RegisterNetEvent('pa-vehicleshop:sendRequestText:server', function(sender, target, price, model, dealershipId)
    TriggerClientEvent('pa-vehicleshop:sendRequestText:client', target, sender, price, model, dealershipId)
end)

RegisterNetEvent('pa-vehicleshop:declinePayment:server', function(sender)
...
AddEventHandler('playerDropped', function()
    for k, v in pairs(testVehicles) do
        if v.playerId == source then
            TriggerClientEvent('pa-vehicleshop:deleteVehicle:client', -1, k)
        end
    end
end)

--RegisterNetEvent('pa-vehicleshop:deleteVehicleShowroom:server', function(dealershipId, spotId, newModel, props)
RegisterNetEvent('pa-vehicleshop:deleteVehicleShowroom:server', function(dealershipId, spotId, newModel)
    for _, playerId in ipairs(GetPlayers()) do
        ...
        if numPlayerId ~= source then
            ...
            if dist <= 40 then
                --TriggerClientEvent('pa-vehicleshop:deleteVehicleShowroom:client', numPlayerId, dealershipId, spotId, newModel, props)
                TriggerClientEvent('pa-vehicleshop:deleteVehicleShowroom:client', numPlayerId, dealershipId, spotId, newModel, true)
            else
                TriggerClientEvent('pa-vehicleshop:deleteVehicleShowroom:client', numPlayerId, dealershipId, spotId, newModel, false)
            end
        end
    end
end)
